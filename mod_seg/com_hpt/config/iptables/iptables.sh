...
iptables -A INPUT -p tcp --syn --dport 22 -m connlimit --connlimit-above $MAX_CONN -j DROP
iptables -A INPUT -p tcp --syn --dport 80 -m connlimit --connlimit-above $MAX_CONN -j DROP
